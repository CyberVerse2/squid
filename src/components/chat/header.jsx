...
function NewIssueModal({setShowIssueModal}) {
	...
	return (
		<Fragment>
			<div className="fixed bg-gray-400 opacity-20 h-screen w-[120%] m-0 top-0 left-0 z-10" onClick={handleCloseModal} />
			<form className="bg-white px-4 pb-4  w-[35%] rounded-md top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 fixed z-20">
				<div className="py-4">
					<h1 className="text-lg text-center text-gray-500 font-semibold">
						New Issue
					</h1>
					<span className="absolute  w-7 p-2 top-3 right-4 rounded-full bg-gray-400 cursor-pointer" onClick={handleCloseModal} >
						<img
							className=" invert brightness-0 w-full block"
					...
					<div>
						...
						>
							{repos.map((repo, index) => (
								<option
									className="indent-1 text-gray-300 text-sm"
									key={index}
								>
									{repo}
								</option>
